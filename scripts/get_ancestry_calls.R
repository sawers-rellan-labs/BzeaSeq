...
cat(paste("Average number of variants per bin:", round(mean(read_freq$n), 1), "\n"))
cat(paste("Median number of variants per bin:", median(read_freq$n), "\n"))
cat(paste("Range of variants per bin:", min(read_freq$n), "to", max(read_freq$n), "\n"))
...
if (nrow(non_zero_bins) > 0) {
  ...
  # Match by CONTIG and POS_BIN to ensure correct assignment
  for (i in 1:nrow(non_zero_bins)) {
    chr <- non_zero_bins$CONTIG[i]
    bin <- non_zero_bins$POS_BIN[i]
    match_idx <- which(read_freq$CONTIG == chr & read_freq$POS_BIN == bin)
...
read_freq_bed <- read_freq %>%
  ...
  select(CONTIG, BIN_START, BIN_END, GENOTYPE, SCORE, ALT_FREQ_NORM)
