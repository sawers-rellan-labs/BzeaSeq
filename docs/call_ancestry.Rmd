...
head(read_freq)
mean_n <- mean(read_freq$INFORMATIVE_VARIANT_COUNT)
sd_n <- sd(read_freq$INFORMATIVE_VARIANT_COUNT)


# Display summary of bins
cat("Total number of bins:", nrow(read_freq), "\n")
cat("Average depth sum:",  mean(read_freq$DEPTH_SUM), "\n")
cat("Average number of informative variants per bin:",mean_n , "\n")
...
read_freq <- read_freq %>%
  mutate(
    ...
    ALT_FREQ_WEIGHTED = ALT_FREQ * (INFORMATIVE_VARIANT_COUNT / mean_n),
    # Cap extreme values (optional)
    # ALT_FREQ_WEIGHTED = pmin(pmax(ALT_FREQ_WEIGHTED, 0), 1)
  )
...
p2 <- ggplot(read_freq, aes(x = ALT_FREQ_WEIGHTED)) +
  ...
  labs(title = "Weighted ALT_FREQ distribution", x = "ALT_FREQ_NORM") +
  theme_minimal()
...
non_zero_bins$log <- log10(non_zero_bins$ALT_FREQ_WEIGHTED) # Log transformation
non_zero_bins$asin <- asin(sqrt(non_zero_bins$ALT_FREQ_WEIGHTED))           # Arcsin transformation
...
cluster_summary <- non_zero_bins %>%
  ...
  summarize(
    mean_ALT_FREQ = mean(ALT_FREQ_WEIGHTED),
    min_ALT_FREQ = min(ALT_FREQ_WEIGHTED),
    max_ALT_FREQ = max(ALT_FREQ_WEIGHTED),
    n_bins = n()
  ) %>%
  ungroup()
...
create_hist <- function(data, method_col, title) {
  ggplot(data, aes(x = ALT_FREQ_WEIGHTED, fill = .data[[method_col]])) +
...
h1 <- create_hist(non_zero_bins, "K", "K-means on weighted ALT_FREQ")
...
h4 <- create_hist(non_zero_bins, "Kgmm", "Gaussian mixture model on weighted ALT_FREQ")
...
```{r chr8, fig.height=5}
# display chromosme *
create_scatter(read_freq %>% filter(CONTIG=="chr8"), "Kasin", paste("K-means on log10 transformed ALT_FREQ (", BIN_SIZE_LABEL, " bins)", sep=""))
```
...
colnames(read_freq)
create_tile(read_freq, "Kgmm", paste("Sample PN15_SID1353\n","arcsin weighted ALT_FREQ) | ", BIN_SIZE_LABEL, " bins", sep=""))
...
# Create breeding matrices with clear column interpretation
...
smooth_ancestry_with_hmm <- function(genotypes, transitions = c(0.96, 0.04)) {
  ...
  # This controls how easily the model will switch states
  trans_prob <- matrix(c(
  ...
  # Higher probability on diagonal (correct calls), lower for errors
  emiss_prob <- matrix(c(
  ...
  hmm <- initHMM(c("REF", "HET", "ALT"), c("0", "1", "2"), 
                startProbs= bc2s3,  # Prior probabilities
...
create_tile(read_freq, "Kgmm_HMM", paste("Sample PN15_SID1353\n","HMM weighted ALT_FREQ) | ", BIN_SIZE_LABEL, " bins", sep=""))
...
read_freq %>%
  ...
  mutate(
    ...
    method = factor(method, 
                   ...
                   labels = c("K-means (weighted)", "weighted HMM",
                              "K-means (log10)", "log10 HMM",
                              "K-means (arcsin)", "log10 HMM",
                              "Gaussian Mixture", "GMM-HMM"))
  ) %>%
...
genotype_freq <- read_freq %>%
  pivot_longer(cols = c(K, Klog, Kasin, Kgmm ,Kgmm_HMM), 
               names_to = "method", 
               values_to = "genotype") %>%
  group_by(method, genotype) %>%
  summarize(
...
expectation <- data.frame( method= "expected",
